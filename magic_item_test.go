...
var _ = Describe("MagicItem", func() {
	It("creates normal items", func() {
		item := gildedrose.NewMagicItem("some-name", 1, 2)
	...
	It("creates Sulfuras", func() {
		item := gildedrose.NewMagicItem("Sulfuras, Hand of Ragnaros", 10, 80)
	...
	It("creates AgedBrie", func() {
		item := gildedrose.NewMagicItem("Aged Brie", 2, 0)
	...
	It("creates Backstage Passes", func() {
		item := gildedrose.NewMagicItem("Backstage passes to a TAFKAL80ETC concert", 15, 20)
	...
	It("creates Conjured items", func() {
		item := gildedrose.NewMagicItem("Conjured Mana Cake", 3, 6)
	...
	Context("when Sulfuras is updated", func() {
		It("does not reduce sell in", func() {
			i := gildedrose.NewMagicItem("Sulfuras, Hand of Ragnaros", 1, 80)
